// Firebase setup with NEW credentials
...
let leaderboard = [];
let coinBalance = 0;
let withdrawalMethod = 'binance'; // Default method
let binanceId = '';
let xrocketUsername = '';
let totalReferred = 0;
let bonusAds = 0;
...
// ==================== COUNTDOWN LOGIC (SEPARATE FOR EACH AD) ====================
...
async function saveWithdrawalInfo(method) {
  if (method === 'binance') {
    ...
    if (!username) return showToast('❌ Please enter your Telegram username');
    ...
    try {
      ...
      showToast('✅ xRocket username saved!');
    } catch (e) {
      showToast('❌ Save failed');
    }
  }
...
async function playAd(adType) {
  ...
  // Check if countdown is running for this specific ad
  if (adType === 'LibTL' && libtlCountdownInterval) {
    showToast('⏳ Please wait for LibTL countdown to finish');
    return;
  }

  if (adType === 'Giga' && gigaCountdownInterval) {
    showToast('⏳ Please wait for Giga countdown to finish');
    return;
  }
  ...
  try {
    ...
    updateDisplay();
    updateYouPage();
    showToast(`✅ Ad watched successfully! +${adsToAdd} 🎥`);
    ...
    // Start countdown for the specific ad network
    if (adType === 'LibTL') {
...
async function withdraw() {
  ...
  // Check if user has saved withdrawal info
  if (withdrawalMethod === 'binance' && !binanceId) {
    return showToast('❌ Please save your Binance ID first');
  }
  
  if (withdrawalMethod === 'xrocket' && !xrocketUsername) {
    return showToast('❌ Please save your Telegram username first');
  }
...
async function init() {
  console.log('🚀 Initializing DogsGiveaway...');
